...
const { buildCheckPageUrl } = require("./utils");
...
class GithubService {
  ...
  createPullRequestComment(report, checkRunId) {
      ...
      .then((response) => {
        if (response.data.length) {
          return this.getOctokitInstance().request('POST /repos/{owner}/{repo}/issues/{issue_number}/comments', {
            ...
            body: buildPullRequestComment(report, this.options, response.data[0].number, checkRunId),
          });
        }
