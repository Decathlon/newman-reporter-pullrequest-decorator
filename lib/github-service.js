...
const getAssociatedPullRequestNumberFromSha = (octokit,repository, commitSha) => {
...
class GithubService {
  ...
  async createPullRequestComment(report){
    getAssociatedPullRequestNumberFromSha(this.getOctokitInstance(), this.options.repo, this.options.refCommit)
    .then((response) => {
      if (response.data.length == 0){
        console.log("[+] No pull request associated with commit sha")
        return;
      } else {
        this.getOctokitInstance().request('POST /repos/{owner}/{repo}/issues/{issue_number}/comments', {
          owner: this.options.repo.split('/')[0],
          repo: this.options.repo.split('/')[1],
