...
const buildOptions = (reporterOptions, newmanOptions) => {
  ...
  return {
    ...
    enablePullRequestComment: reporterOptions.pullrequestEnableComment,
  };
};
...
class PullRequestDecoratorReporter {
  ...
  decoratePullRequest = (report, options) => {
    this.githubService.createPullRequestCheck(report,options)
  ...
  beforeDone(error, args) {
    ...
    if (!this.options.export) {
      return this.decoratePullRequest(this.buildReport(this.context.assertions, summary.collection.items), this.options);
    }
    return Promise.resolve();
  }
