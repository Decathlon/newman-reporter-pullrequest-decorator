...
const evaluateCollection = (items) => {
  if (items[0].subItems == undefined){ // in the case that there is only one folder
  ...
  } else if (isLeafLevel(items[0].subItems)) { // in the case that there are multiple folders
    console.log("is leaf, stopping");
    let requestsSkippedCount = 0, requestsPassCount = 0;  
    let requestsFailed = [];
    ...
    items.forEach((item) => evaluateCollection(item));
    console.log("is not leaf, progressing");
  }
}
