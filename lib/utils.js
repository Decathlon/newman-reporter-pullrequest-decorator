...
const buildPullRequestComment = (report, options, issueNumber, checkRunId) => {
  ...
  let comment = `Please find the complete report [here](${buildCheckPageUrl(options,issueNumber,checkRunId)})  
  ...
  return requestsFailedCount > 0 ? buildFailureComment(comment, report) : ("Congratulations, All Tests have Passed! \n" + comment);
};

const buildFailureComment = (comment, report) => {
  report.items
