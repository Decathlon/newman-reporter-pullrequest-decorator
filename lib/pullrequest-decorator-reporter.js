...
const decoratePullRequest = (githubService, report, options) => {
  return Promise.allSettled([githubService.createPullRequestComment(report)
    .catch((exception) => console.log(`[+] An error occured : ${exception}`))
    .finally(console.log("[+] Finished adding pull request comment report")),
    githubService.createPullRequestCheck(report,options)
    .catch((exception) => console.error(`[+] An error occured : ${exception}`))
    .finally(() => {
      console.log(`[+] Finished collection: ${options.collectionName}`);
    })]
  )
}
...
class PullRequestDecoratorReporter {
  ...
  beforeDone(error, args) {
    ...
    if (!this.options.export) {
      return decoratePullRequest(this.githubService, this.buildReport(this.context.assertions, summary.collection.items), this.options);
    }
